...
/**
 * Created by HuangMei on 2016/12/6.
 */
...
//		reset();
    ...
    private void animateScroll(int position, float positionOffset){
    ...
    protected void animateTablet(View left, View right, float positionOffset){
        if (mState != State.IDLE){
            ...
            if (right != null){
                ...
                ViewHelper.setPivotX(right, right.getMeasuredWidth() / 2);
    ...
    private void animateFlipHorizontal(View left, View right, float positionOffset, int positionOffserPixels){
        if (mState != State.IDLE){
            if (left != null){
                manageLayer(left, true);
            ...
            if (right != null){
                ...
                if (mRot > 90.0f){
                    ...
                    mTrans = -getWidth() - getPageMargin() + positionOffserPixels;
                    ViewHelper.setPivotX(right, right.getMeasuredWidth() * 0.5f);
                    ViewHelper.setPivotY(right, right.getMeasuredHeight()* 0.5f);
                    ViewHelper.setTranslationX(right, mTrans);
                    ViewHelper.setRotationY(right, mRot);
    ...
    private void manageLayer(View v, boolean enableHardware){
        if (!API_11)
            return;
        int layType = enableHardware ? View.LAYER_TYPE_HARDWARE : View.LAYER_TYPE_NONE;
        if (layType != v.getLayerType())
            v.setLayerType(layType, null);
    }
    ...
    private Matrix mMatricx = new Matrix();
    private Camera mCarema = new Camera();
    private float[] mTempFloat2 = new float[2];

    protected float getOffsetXForRotation(float degrees, int width, int height){
        mMatricx.reset();
        mCarema.save();
        mCarema.rotateY(Math.abs(degrees));
        mCarema.getMatrix(mMatricx);
        mCarema.restore();

        mMatricx.preTranslate(-width * 0.5f, -height * 0.5f);
        mTempFloat2[0] = width;
        mTempFloat2[1] = height;
        mMatricx.mapPoints(mTempFloat2);
